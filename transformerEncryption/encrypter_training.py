...
def point_wise_feed_forward_network(d_model, dff):
  return tf.keras.Sequential([
      tf.keras.layers.Dense(dff, activation=gelu, kernel_initializer='he_uniform', kernel_regularizer='l2'),  # (batch_size, seq_len, dff)
      tf.keras.layers.Dense(d_model, activation=gelu, kernel_initializer='he_uniform', kernel_regularizer='l2')  # (batch_size, seq_len, d_model)
  ])
...
checkpoint_path = "./checkpoints/encrypter"
...
for epoch in range(EPOCHS):
  ...
  for (batch, (inp, tar, pos)) in enumerate(train_data):
  ...
  for (batch, (inp, tar, pos)) in enumerate(val_data):
...
# with open('logs/encrypter_loss_accuracy.pulickle','wb') as en_file:
#     pickle.dump([train_loss_result,train_accuracy_result,validate_loss_result,validate_accuracy_result],en_file)
